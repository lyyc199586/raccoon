...
# wave speed
cs = '${fparse sqrt(G/rho)}' # shear wave speed
cr = '${fparse cs*(0.862+1.14*nu)/(1+nu)}' # rayleigh wave speed

l = 1
r = 5
# u0 = 0.01 # top disp
p = 1 # top taction
# Tc = 11.6 # crack start time
Tc = 12
Tf = 50 # final time
...
[Postprocessors]
  ...
  [tip_vel]
    type = ParsedPostprocessor
    expression = 'if(t>Tc, cr*(1- Gc/2/Jint), 0)'
    pp_names = 'Jint'
    constant_names = 'Tc cr Gc'
    constant_expressions = '${Tc} ${cr} ${Gc}'
    use_t = true
  []
  [tip_adv]
    type = ParsedPostprocessor
    expression = 'dt*tip_vel'
    pp_names = 'dt tip_vel'
  []
  # [tip_adv]
  #   type = ParsedPostprocessor
...
[Outputs]
  exodus = true
  # file_base = './out/sharp_static_p${p}_Tc${Tc}_Tf${Tf}/sharp'
  file_base = './out/sharp_p${p}_Tc${Tc}_Tf${Tf}/sharp'
  [csv]
    type = CSV
    # file_base = './gold/sharp_static_p${p}_Tc${Tc}_Tf${Tf}'
    file_base = './gold/sharp_p${p}_Tc${Tc}_Tf${Tf}'
  []
[]