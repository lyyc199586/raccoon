...
HistoryFieldBoundsAux::validParams()
{
  ...
  // params.addRequiredParam<VariableName>("history_variable", "The history variable to get the
  // value of.");
  params.addRequiredParam<std::string>("history_variable", "The variable to get the value of.");
...
HistoryFieldBoundsAux::HistoryFieldBoundsAux(const InputParameters & parameters)
  : BoundsAuxBase(parameters),
    ...
    _hist_var(_subproblem.getStandardVariable(_tid, getParam<std::string>("history_variable"))),
    // _hist_var(
    //     _subproblem
    //         .getVariable(_tid, getParam<std::string>("history_variable"), Moose::VAR_AUXILIARY)
    //         ),
    _serialized_solution(_aux_sys.currentSolution()),
...
HistoryFieldBoundsAux::getBound()
{
  ...
  for (const auto near_node_id : _node_to_near_nodes_map[_current_node->id()])
  {
    ...
    Real d_hist = _hist_var.getNodalValue(near_node); // only valid for serial
    if (d_hist > d_max)
    {
      d_max = d_hist;
      // for debug
