...
#include "Function.h"
#include "NucleationMicroForceDebug.h"

registerADMooseObject("raccoonApp", NucleationMicroForceDebug);

InputParameters
NucleationMicroForceDebug::validParams()
{
  ...
  params.addParam<MaterialPropertyName>("tensile_strength", "sigma_ts", "The tensile strength of the material beyond which the material fails");
...
NucleationMicroForceDebug::NucleationMicroForceDebug(const InputParameters & parameters)
  : Material(parameters),
...
void
NucleationMicroForceDebug::computeQpProperties()
{
  // The bulk modulus
  ...
  ADReal gamma_1 = (1.0 + _delta) / (2.0 * _sigma_ts[_qp] * _sigma_cs);
  ...
  ADReal beta_1 = (-gamma_1 * M - gamma_2) * (_sigma_cs - _sigma_ts[_qp]) -
                  gamma_0 * (pow(_sigma_cs, 3) - pow(_sigma_ts[_qp], 3));
  ADReal beta_2 = std::sqrt(3.0) * ((-gamma_1 * M + gamma_2) * (_sigma_cs + _sigma_ts[_qp]) +
                                    gamma_0 * (pow(_sigma_cs, 3) + pow(_sigma_ts[_qp], 3)));
  ...
  _f_sigma[_qp] = sqrt(J2) + (_sigma_cs-_sigma_ts[_qp])/sqrt(3)/(_sigma_cs+_sigma_ts[_qp]) * I1 - 2*_sigma_cs*_sigma_ts[_qp]/sqrt(3)/(_sigma_cs+_sigma_ts[_qp]);
}
