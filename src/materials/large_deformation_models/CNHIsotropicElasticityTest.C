...
#include "CNHIsotropicElasticityTest.h"
#include "RaccoonUtils.h"

registerMooseObject("raccoonApp", CNHIsotropicElasticityTest);

InputParameters
CNHIsotropicElasticityTest::validParams()
{
...
CNHIsotropicElasticityTest::CNHIsotropicElasticityTest(const InputParameters & parameters)
...
ADRankTwoTensor
CNHIsotropicElasticityTest::computeMandelStress(const ADRankTwoTensor & Fe,
...
ADRankTwoTensor
CNHIsotropicElasticityTest::computeMandelStressNoDecomposition(const ADRankTwoTensor & Fe,
                                                           const bool plasticity_update)
{
  // We use the left Cauchy-Green strain
  ADRankTwoTensor left_strain;
  ADRankTwoTensor right_strain;
  if (plasticity_update)
  {
    ADRankTwoTensor expFe = RaccoonUtils::exp(Fe);
    left_strain = expFe * expFe.transpose();
    right_strain = expFe.transpose() * expFe;
  }
  else
    left_strain = Fe * Fe.transpose();
    right_strain = Fe.transpose() * Fe;

  ADReal J = std::sqrt(right_strain.det());
  ADReal lambda = _K[_qp] - 2./3. * _G[_qp];
  ...
  // Kirchhoff stress tau = J * {\partial{W}\over\parital{F}} * F^T
  ADRankTwoTensor stress_intact = _G[_qp] * (left_strain - I2) + lambda * (J - 1) * I2;
  ADRankTwoTensor stress = _g[_qp] * stress_intact;
  ...
  if (!plasticity_update)
  {
    ...
    // W(F) = G/2*(I1 - 3) - G*ln(J) + lambda/2*(J-1)^2
    _psie_active[_qp] = 0.5*_G[_qp]*(right_strain.trace() - 3.0) - _G[_qp] * std::log(J) + 0.5*lambda*(J - 1)*(J - 1);
