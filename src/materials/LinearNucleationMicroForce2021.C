...
LinearNucleationMicroForce2021::validParams()
{
  ...
  params.addParam<bool>("if_stress_intact", false, "Flag");
...
LinearNucleationMicroForce2021::LinearNucleationMicroForce2021(const InputParameters & parameters)
  : Material(parameters),
    ...
    _if_stress_intact(getParam<bool>("if_stress_intact"))
{
}
...
LinearNucleationMicroForce2021::computeQpProperties()
{
  ...
  if (_if_stress_intact)
  {
    _ex_driving[_qp] =
        (beta_2 * std::sqrt(J2) + beta_1 * I1 + beta_0) * _g[_qp] +
        (1.0 - std::sqrt(I1 * I1) / I1) * std::sqrt(_g[_qp]) *
            (J2 / 2.0 / _mu[_qp] + I1 * I1 / 6.0 / (3.0 * _lambda[_qp] + 2.0 * _mu[_qp]));
    _stress_balance[_qp] =
        pow(_g[_qp], 2) * (J2 / _mu[_qp] + pow(I1, 2) / 9.0 / K) - _ex_driving[_qp] - M;
  }
  else
  {
    _ex_driving[_qp] =
