#include "ConditionalHistoryBoundsAux.h"
#include "KDTree.h"
#include "MooseMesh.h"
...
registerMooseObject("raccoonApp", ConditionalHistoryBoundsAux);

InputParameters
ConditionalHistoryBoundsAux::validParams()
{
  ...
  params.addRequiredParam<Real>(
      "threshold_value", "The threshold value for conditional history bound for the variable");
  params.addRequiredParam<Real>("search_radius",
                                "The search radius for the maximum history field value");
  params.addRequiredParam<AuxVariableName>("history_variable", "The variable to get the value of.");
  params.set<MooseEnum>("bound_type") = "lower";
  params.suppressParameter<MooseEnum>("bound_type");
  return params;
}

ConditionalHistoryBoundsAux::ConditionalHistoryBoundsAux(const InputParameters & parameters)
  : BoundsAuxBase(parameters),
    ...
    _hist_var(_subproblem.getStandardVariable(_tid, getParam<AuxVariableName>("history_variable"))),
...
Real
ConditionalHistoryBoundsAux::getBound()
{
  ...
  if (d_max <= _threshold_value)      // H <= d_c
    return d_old;
  else if (d_old <= _threshold_value) // H > d_c and d_n <= d_c
    return _fixed_bound_value;
  else                                // H > d_c and d_n > d_c
    return d_old;
}
